...
async def pay_with_click(cb: CallbackQuery):
    t = lambda k: get_text(cb.from_user.id, k)
    cart = user_carts.get(cb.from_user.id)
    if not cart or not cart["items"]:
        await cb.answer(t("empty_cart"), show_alert=True)
    ...
    # Narxlarni tiyinlarda hisoblash (1 so'm = 100 tiyin)
    prices = []
    for name, count in counter.items():
        price = next(item["price"] for item in cart["items"] if item["name"] == name)
        prices.append(LabeledPrice(label=f"{name} × {count}", amount=price * count * 100))

    # Invoice yuborish
    await bot.send_invoice(
        chat_id=cb.from_user.id,
        title="Kassir Bot — To'lov",
        description="Sotib olingan mahsulotlar uchun to'lov",
        payload=f"payment_{cb.from_user.id}_{datetime.now().timestamp()}",
        provider_token=PAYMENTS_PROVIDER_TOKEN,
        currency="UZS",
        prices=prices,
        start_parameter="kassir-payment",
        need_name=False,
    ...
    await cb.answer("To'lov sahifasi ochilmoqda...")
...
async def successful_payment(message: Message):
    ...
    if not cart or not cart["items"]:
        await message.answer("Xato: Savat ma'lumotlari topilmadi.")
        return
    ...
    # Qoldiqni kamaytirish va sotuvni yozish
    for name, count in counter.items():
        product = next((p for p in products if p["name"] == name), None)
        if product:
            product["sani"] -= count
    ...
    await message.answer(
        t("payment_accepted").format(total=total),
        reply_markup=main_menu(user_id)
    )
    await message.answer(t("sell_again"), reply_markup=main_menu(user_id))
